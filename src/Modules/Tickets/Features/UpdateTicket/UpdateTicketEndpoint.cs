...
namespace OpenPsa.Modules.Tickets.Features.UpdateTicket;

public class UpdateTicketEndpoint : IEndpointFeature {
    public static void MapEndpoint(IEndpointRouteBuilder app) {
        app.MapPut("/api/tickets/{id:guid}", async (Guid id, UpdateTicketRequest request, OpenPsaDbContext db, IMessageBus bus, CancellationToken ct) => {
            var ticket = await db.Set<Ticket>().FirstOrDefaultAsync(t => t.Id == id, ct);
            if (ticket == null) return Results.NotFound();

            var wasResolved = ticket.Status is TicketStatus.Resolved or TicketStatus.Closed;
            var isNowResolved = request.Status is TicketStatus.Resolved or TicketStatus.Closed;
            ...
            if (!wasResolved && isNowResolved) ticket.ResolvedAt = DateTime.UtcNow;
            else if (wasResolved && !isNowResolved) ticket.ResolvedAt = null;
        ...
        }).RequirePermission("tickets.update").WithTags("Tickets");
    }
}
